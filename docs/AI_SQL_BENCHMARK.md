...
Benchmark model calls carry an explicit purpose marker and are accepted only when the backend resolves the current database user as an administrator. They use a separate audited `ai_benchmark_query` allowance (2,500 model calls per day by default, configurable with `ADMIN_BENCHMARK_DAILY_LIMIT`) and a benchmark-specific minute window. Ordinary user quotas and normal AI SQL traffic are unchanged. Because the production route uses three model calls per question and an unexpected empty result may use one semantic-repair call, confirm the configured allowance before starting a large run. Any deterministic or zero-token result is labelled `llm_unavailable`; it is never counted as model-backed accuracy evidence. The report separates model-backed execution accuracy, provider availability, completed-manifest coverage, executable SQL, and answer-contract diagnostics so an outage or presentation check cannot be mistaken for answer accuracy.
