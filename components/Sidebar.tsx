...
import { Layout, Database, Play, Search, Upload, X, BarChart2, MessageSquare, LogOut, Users, Crown, Pencil, Eye, GitMerge, Wrench, Sparkles, KeyRound, Check, AlertTriangle, Loader2, ChevronRight, Lightbulb, Bell, Activity, Zap, Shield, Gamepad2, Settings, Wand2, FlaskConical, SlidersHorizontal } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, onToggle, onOpenUserManagement, hasVisualResult, onDataStory, onOpenTabManager }) => {
    ...
    const analysisSection = [
        ...
        ...(userRole === UserRole.ADMIN ? [{ id: Tab.PARAMETERS, label: 'Parameters', icon: SlidersHorizontal, tooltip: 'The deterministic query engine, made visible — every Question Builder knob AI SQL can map a question to (so it runs deterministically instead of via the LLM), plus a live in-browser mapping demo. Admin only.' }] : []),
