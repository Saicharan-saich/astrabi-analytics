...
Evidence: expense refers to expense_description; spend more than fifty dollars on average refers to DIVIDE(SUM(cost), COUNT(expense_id)) > 50`;
    ...
    it('does not confuse a no-issue status phrase with missing related records', () => {
        const loanModel: SemanticModel = {
            ...
            fields: [
                ...
                { ...field('amount', 'metric', 'currency'), synonyms: ['loan amount'] },
            ],
        };
    ...
    it('recognises a group with the most number of members as a frequency ranking', () => {
        const channelModel: SemanticModel = {
            ...
            fields: [
                field('Country', 'dimension', 'category'),
                { ...field('channel_id', 'dimension', 'identifier'), synonyms: ['TV channel'], distinctCount: 15 },
            ],
        };
