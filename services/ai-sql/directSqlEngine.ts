...
- When several calculations are requested for the same grouped measure (for example sales, percent of total sales, rank, and cumulative sales percentage), aggregate that measure once at the named group grain in a base CTE. Derive every requested calculation from that same grouped measure. Percent-of-total sales uses SUM(sales), never COUNT(rows), unless the question explicitly asks for share of rows/orders.
